...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Admission Trade</h3>
        <div class="card-tools">
          <a class="btn btn-primary" href="{{ route('admission_trade.create')}}">New Item</a>
        </div>
    </div>
    <div class="card-body">
      <div class="table-responsive">
        <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
                @foreach ($admission_trades as $item)
                <tr>
                    <td>{{$item->id}}</td>
                    <td>{{$item->name}}</td>
                    <td>{{$item->code}}</td>
                    <td>{{$item->admission_class? $item->admission_class->name:''}}</td>
                      ...
                      <div class="btn-group btn-group-sm">
                        ...
                        <div class="dropdown-menu">
                          ...
                          <div class="dropdown-divider"></div>
                            <a href="{{route('admission_trade.edit',$item->id)}}" class="dropdown-item"><i class="fas fa-edit"></i> Edit</a>
                            <div class="dropdown-divider"></div>
                            <form class="delete" action="{{ route('admission_trade.destroy',$item->id) }}" method="post">
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}
